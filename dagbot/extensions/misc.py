...
class misc(commands.Cog):
    ...
    async def suggest(self, ctx, *, suggest):
        ...
        embed.add_field(
            name="SENT", value="Your SUGGESTION has been added to the Support server"
        )
        embed.add_field(
    ...
    async def bug(self, ctx, *, bug):
        ...
        embed.add_field(
            name="SENT", value="Your error has been sent to my creator's bug channel"
        )
        embed.add_field(
            name="Support Server", value="[Join Now](https://discord.gg/5Y2ryNq)"
        )
    ...
    async def serverinfo(self, ctx):
        ...
        role_list = " ".join(
            role.mention for role in roles[::-1][:10] if role.id != ctx.guild.id
        )
        embed = discord.Embed(
            colour=ctx.guild.me.color,
            title=f"{guild}",
            description=f"**Owner:** {ctx.guild.owner.mention}\n**Region:** {region}\n**Guild created**: {humanize.naturaltime(datetime.utcnow() - ctx.guild.created_at)}",
        )
        ...
        embed.add_field(
            ...
            value=f"<:online:724328584621064193>: **{humanize.intcomma(online)}**\n<:offline:724328584751349903>: **{humanize.intcomma(offline)}**\n<:dnd:724328585078243438>: **{humanize.intcomma(dnd)}**\n<:idle:724328584893956097>: **{humanize.intcomma(idle)}**",
        )
        embed.add_field(
    ...
    async def userinfo(self, ctx, user=None):
        ...
        if user.bot:
            ...
            botthing = "\u0020"
        status_list = (
            f"{sl[str(user.status)]}{mlsl[str(user.mobile_status)]}{wlsl[str(user.web_status)]}{dlsl[str(user.desktop_status)]}"
            + botthing
        )
        embed = discord.Embed(
    ...
    async def spotify(self, ctx, *, user=None):
        ...
        # old bar: █ ░
        pbar = f"`{minseccur}`| {(pcent-1)*'─'}⚪️{(25-pcent)*'─'} | `{minsecdur}`"
    ...
    async def about(self, ctx):
        embed = discord.Embed(
            title="About Dagbot",
            description="A ~~low effort and kinda useless~~ bot, that does memes. With a bunch of hapahazardly thrown together features dagbot should fullfill the need to entertain."
        ...
        versions = f"<:python:737012280037736550> Python {platform.python_version()}\n<:dpy:737012375747821650> discord.py {discord.__version__}"
    ...
    async def credits(self, ctx):
        return await ctx.send(
            "Support Server my duded! Join and dm to get credit! This command will come soon"
        )
    ...
    async def source(self, ctx, *, command: str = None):
        ...
        if command is None:
            ...
            if com is None:
                return await ctx.send('There is no command with that name. Maybe check the repo\nhttps://github.com/Daggy1234/dagbot')
            else:
                code = com.callback.__code__
                module = com.callback.__module__
                filename = code.co_filename
                lines, firstline = inspect.getsourcelines(code)
                location = os.path.relpath(filename).replace('\\', '/')
                final_url = f'{repo}/blob/master/{location}#L{firstline}-L{firstline + len(lines) - 1}'
